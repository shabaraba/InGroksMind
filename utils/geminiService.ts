...
export const getGeminiAnswer = async (
  ...
  try {
    ...
    return {
      content: locale === 'ja'
        ? `${useGemini ? '（API呼び出しエラーのためモック回答を表示しています）' : '（API機能は無効になっています）'}これはGeminiの模範解答です。${quiz.content_ja}について、${style.name_ja}の口調でお答えします。このお題についての正確な情報をご提供します。実際の情報に基づいて回答すると、このようになるでしょう。`
